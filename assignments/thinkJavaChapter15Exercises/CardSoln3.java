...
public class CardSoln3 {
	...
	// n number poker hands
	Deck fiftyTwo = new Deck();
	fiftyTwo.shuffle();
	int n = 4;
	Deck[] hands = new PokerHand[n];
	for(int i = 0; i<1; i++){
		System.out.println("Poker Hand " + i+1 + ":");
		hands[i] = fiftyTwo.deal();
		// hands[i].print();
	}
...
class Deck {
	...
	return merge(d1, d2);
		...
		public Deck deal(){
			int numCards = 5;
			Deck sub = new Deck();
			Deck myHand = new PokerHand(numCards);
			sub.shuffle();
			for (int i = 0; i<numCards; i++) {
					sub.cards[i] = myHand.cards[i];
			}
			return myHand;
		}
}
