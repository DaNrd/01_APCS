...
public class GameFrame extends JComponent{
    ...
    public int deadMho = 0; // counts the number of dead mhos
    ...
    public void init(){ // called when game starts
        ...
        deadMho = 0; // reset dead mho cound
    ...
    public void spawnMho(){ // spawns mhos
        for(int i = 0; i < mhoCount; i++){ // runs as many times as there are mhos
            while(true){
                int gridPosx = (int) (10 * Math.random()); // rand x
                int gridPosy = (int) (10 * Math.random()); // rand y
                if(objectLoc[gridPosx+1][gridPosy+1] == 0){
    ...
    public void updateFrameSize() { // resize
    ...
    public void jump(){ // moves player randomly but never on to fence
        ...
        while (objectLoc[x][y] == 1) { // this is the never on fence part
          x = (int) (10 * Math.random());
          y = (int) (10 * Math.random());
        }
        checkWin();
    ...
    public void findNpcs() { // npc moves and check how many npc die
        int oldDeadMho = deadMho;
        deadMho = 0;
        for(int i = 0; i < npcLoc.length; i++) {
            if (objectLoc[npcLoc[i].xpos][npcLoc[i].ypos] != 1){
              npcLoc[i].npcMove(); // moves the npcs in order
            }
            if (objectLoc[npcLoc[i].xpos][npcLoc[i].ypos] == 1){ // if mho is on fence
              deadMho++; // adds to dead mho cound
            }
        }
        if (deadMho > oldDeadMho){
          score = score + roundNum*(deadMho - oldDeadMho);
        }
        checkWin();
    }
    ...
    public void checkWin(){ // check if player is on mho or fence or if all mhos are dead
      ...
      // messages
      if (deadMho == mhoCount){ // if all mhos are dead
        String s = "";
        if (roundNum > 1){
          s = "s";
        }
        message = "You Won! You have passed " + roundNum + " round" + s + " and have " + score + " points! Press any button to keep going!";
        reset = true;
